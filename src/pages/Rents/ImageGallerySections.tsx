...
// Image Modal Component - UPDATED to use both thumbnail and full image
...
const ImageGallerySection = ({ villa }) => {
  ...
  // -------- Extract Data Safely ----------
  // Create a single array with both thumbnail and full image URLs
  const media_images =
    villa.media_images?.map((img) => ({
      id: img.id,
      // For gallery display - use thumbnail (smaller size)
      thumbnail: img.thumbnail || img.file_url || LOCAL_FALLBACK,
      thumbnail_url: img.thumbnail || img.file_url || LOCAL_FALLBACK,
      // For modal/slider display - use full image
      image: img.image || img.file_url || LOCAL_FALLBACK,
    ...
    try {
      ...
      if (media_images.length > 0) {
        ...
        for (let i = 0; i < imagesToUse.length; i++) {
          ...
          try {
            // Use full image URL for PDF
            const fullImageUrl = imagesToUse[i].image || imagesToUse[i].url || imagesToUse[i].thumbnail;
            const imgEl = await loadImageWithFallback(fullImageUrl);
            if (imgEl) {
      ...
      Swal.fire({
        ...
        text: "Property Brochure Download Successfully!",
        timer: 2000,
      });
    } catch (err) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-7">
          ...
          <div className="mt-8 text-center">
            ...
            <Description
              ...
              descriptionImage={
                media_images?.[1]?.thumbnail ||
                media_images?.[1]?.thumbnail_url ||
        ...
        <div className="lg:col-span-5 border-l lg:pl-12 pl-0">
              ...
              {interior_amenities.length > 0 && (
                <>
                  ...
                  {interior_amenities.length > 4 && (
                    <button
                      onClick={() => setShowAllInterior(!showAllInterior)}
          ...
          <button
            ...
            className="w-full bg-teal-600 cursor-pointer hover:bg-teal-700 text-white font-semibold py-4 px-8 rounded-lg text-lg mt-8"
          >
      ...
      {isRentType && (
        <>
          <RatesBookingInformation
            ...
            damage_deposit={villa?.damage_deposit}
          />
          <div className="">
