...
// Utility function to transform backend errors to user-friendly messages
const transformBackendErrors = (errorData: Record<string, any>): Record<string, any> => {
  const transformed: Record<string, any> = { ...errorData };
  
  // Transform email errors
  if (errorData.email) {
    if (Array.isArray(errorData.email)) {
      transformed.email = errorData.email.map((err: string) => {
        const lowerErr = err.toLowerCase();
        if (lowerErr.includes('unique') || 
            lowerErr.includes('already') ||
            lowerErr.includes('exist') ||
      ...
      const lowerErr = errorData.email.toLowerCase();
      if (lowerErr.includes('unique') || 
          lowerErr.includes('already') ||
          lowerErr.includes('exist') ||
          lowerErr.includes('taken')) {
        transformed.email = ["Email already exists. Please use a different email or try logging in."];
      } else {
        transformed.email = [errorData.email];
      }
    }
  }
  
  // Transform generic field errors
  if (errorData.non_field_errors) {
    if (Array.isArray(errorData.non_field_errors)) {
      transformed.non_field_errors = errorData.non_field_errors.map((err: string) => {
        const lowerErr = err.toLowerCase();
        if (lowerErr.includes('unique') || lowerErr.includes('already') || lowerErr.includes('exist')) {
          return "This email address is already registered. Please use a different email or try logging in.";
        }
        return err;
      });
    } else if (typeof errorData.non_field_errors === 'string') {
      const lowerErr = errorData.non_field_errors.toLowerCase();
      if (lowerErr.includes('unique') || lowerErr.includes('already') || lowerErr.includes('exist')) {
        transformed.non_field_errors = ["This email address is already registered. Please use a different email or try logging in."];
      } else {
        transformed.non_field_errors = [errorData.non_field_errors];
      }
    }
  }
  
  // Ensure all error fields are arrays for consistent display
...
const Register: React.FC = () => {
  ...
  // Handle email input - clear error when typing
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (register.fulfilled.match(resultAction)) {
      ...
      if (resultAction.payload && typeof resultAction.payload === "object") {
        ...
        const transformedErrors = transformBackendErrors(errorData);
        setValidationErrors(transformedErrors);

        // Extract error messages for display
        let errorMessages: string[] = [];
        
        // Check email errors first (for duplicate email case)
        if (transformedErrors.email && transformedErrors.email.length > 0) {
          errorMessages = errorMessages.concat(transformedErrors.email);
        }
        
        if (transformedErrors.password && transformedErrors.password.length > 0) {
          errorMessages = errorMessages.concat(transformedErrors.password);
        }
        
        if (transformedErrors.non_field_errors && transformedErrors.non_field_errors.length > 0) {
          errorMessages = errorMessages.concat(transformedErrors.non_field_errors);
        }
        ...
        if (errorMessages.length > 0) {
          ...
          const isEmailDuplicate = errorMessage.toLowerCase().includes('already exists') || 
                                   errorMessage.toLowerCase().includes('already registered');
          const title = isEmailDuplicate ? "Email Already Registered" : "Registration Failed";
