...
const AdminPropertiesRentals: React.FC = () => {
  ...
  const slice: any = useSelector((s: RootState | any) => s.propertyBooking);
  ...
  // normalized array from redux (then filtered to rent only)
  const reduxArrayRaw = useMemo(() => toArray(rawProperties), [rawProperties]);

  // Filter redux array to only include rent listing_type (various possible keys checked)
  const reduxRentOnly = useMemo(() => {
    return reduxArrayRaw.filter((p: any) => {
      const lt = String(p.listing_type ?? p.rateType ?? p.type ?? p.property_type ?? "").toLowerCase();
      return lt === "rent";
    });
  }, [reduxArrayRaw]);
  ...
  // sync localProperties whenever redux rent-only array changes
  useEffect(() => {
    setLocalProperties(reduxRentOnly.map((p: any) => ({ ...p })));
  }, [reduxRentOnly]);

  useEffect(() => {
    // dispatch fetchProperties with a filter param so server returns rent-only items (if your thunk supports it)
    // If your thunk expects a different signature, change this call accordingly.
    const p = dispatch(fetchProperties({ params: { listing_type: "rent" } }));
    p
      .then((res: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          // refresh redux list to be safe/in-sync with rent filter
          dispatch(fetchProperties({ params: { listing_type: "rent" } }));
        } catch (err: any) {
  ...
  const handleModalSave = async () => {
    ...
    try {
      ...
      // refresh redux store with rent filter
      dispatch(fetchProperties({ params: { listing_type: "rent" } }));
    } catch (err: any) {
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{item.details ?? (item.description ? String(item.description).slice(0, 20) : "")}</div>
                      </div>
                    </td>
