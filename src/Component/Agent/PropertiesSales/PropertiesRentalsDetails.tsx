...
//     // console.log('property data by tbc', property?.tbc_by);
//     return (
...
//    {dynamicCalendarLink && (
//       <button
//         onClick={() => navigator.clipboard.writeText(dynamicCalendarLink)}
...
const PropertiesRentalsDetails: FC = () => {
  ...
  // Function to download images via server API - DIRECT DOWNLOAD
  const downloadAllImagesAsZip = async () => {
    if (!property?.id) return;
    
    // Direct server API call - no progress bar, no modal
    try {
      const response = await fetch(
        `${API_BASE}/download/download-images/${property.id}`,
        {
          method: 'POST',
          headers: {
      ...
      // If server endpoint fails, download individual images
  ...
  const downloadAllVideosAsZip = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/download/download-videos/${property.id}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      // If server endpoint fails, download individual videos
      downloadVideosIndividually();
    }
  };

  // Fallback: Download images individually
  const downloadImagesIndividually = () => {
    if (propertyImages.length === 0) {
      showActionMessage('No images available to download.');
      return;
    }
    
    // Open each image in new tab for direct download
    propertyImages.forEach((img, index) => {
      const imageUrl = img.image.startsWith('/') 
        ? `${API_BASE.replace(/\/api\/?$/, '')}${img.image}`
        : img.image;
    ...
    Swal.fire({
      ...
      text: `${propertyImages.length} images download started. Check browser download manager.`,
      timer: 3000,
      showConfirmButton: true,
    });
  };

  // Fallback: Download videos individually
  const downloadVideosIndividually = () => {
    if (propertyVideos.length === 0) {
      showActionMessage('No videos available to download.');
      return;
    }
    
    // Open each video in new tab for direct download
    propertyVideos.forEach((video, index) => {
      const videoUrl = video.video.startsWith('/')
        ? `${API_BASE.replace(/\/api\/?$/, '')}${video.video}`
        : video.video;
      
      // Create download link for each video
      const a = document.createElement('a');
      a.href = videoUrl;
    ...
    showActionMessage(`${propertyVideos.length} videos download started. Check browser download manager.`);
  };
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-50 font-sans">
      <div className="">
        ...
        <div>
       Checking in More Details and the property card below 
        </div>
