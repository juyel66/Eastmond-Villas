 ...
 * - Fetches all properties pages from: http://10.10.13.60:8000/api/villas/properties/?page=1
...
export default function ManageProperties({ agentId = AGENT_ID_DEFAULT, agentName = "John Smith" }: ManagePropertiesProps) {
  ...
  async function fetchAllPropertiesPages() {
    ...
    try {
      ...
      while (url) {
        ...
        if (json.next) {
          url = json.next.startsWith("http") ? json.next : `${API_BASE}${json.next}`;
        } else {
          url = null;
        }
      }
      ...
      const preselected = normalized.filter((p) => Number(p.assigned_agent) === Number(agentId)).map((p) => p.id);
  ...
  const handleToggleProperty = (propertyId: number) => {
    setSelectedPropertyIds((prev) => (prev.includes(propertyId) ? prev.filter((id) => id !== propertyId) : [...prev, propertyId]));
  };
  ...
  async function patchAssignedAgent(propertyId: number, assignedValue: number | null) {
    ...
    fd.append("assigned_agent", assignedValue === null ? "" : String(assignedValue));
  ...
  async function handleSaveAssignments() {
    ...
    try {
      ...
      for (const id of toAssign) requests.push(patchAssignedAgent(id, Number(agentId)));
      ...
      <div className="w-full bg-white rounded-xl p-6 md:p-8">
        ...
        <footer className="flex justify-between items-center pt-4 border-t border-gray-100">
          ...
          <div className="flex space-x-3">
            <button className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-100 transition" onClick={handleCancel}>
              Cancel
            </button>
            <button className="px-6 py-2 flex items-center bg-teal-600 text-white rounded-lg font-medium shadow-md hover:bg-teal-700 transition" onClick={handleSaveAssignments} disabled={saving}>
