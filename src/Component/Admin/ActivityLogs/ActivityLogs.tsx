...
//                    <span
//   style={{
//     background: typeBadgeColor(row.type).bg,
//     color: typeBadgeColor(row.type).txt,
//     padding: "6px 10px",
//     borderRadius: 999,
//     fontSize: 12,
//     fontWeight: 600,
...
export default function ActivityLogsExactPDF_NoBorders({ headerImageUrl }) {
  ...
  function toggleSelectAllVisible() {
    const indices = currentPageData.map((_, i) => startIndex + i);
    const allSelected = indices.every((i) => selectedSet.has(i));
  ...
  async function exportExactPDF_NoBorders() {
    ...
    // Get all selected rows from filtered data (all pages)
    const indices = Array.from(selectedSet).sort((a, b) => a - b);
    const rows = indices.map((i) => filtered[i]).filter(Boolean);
  ...
  function isSelected(index) {
    const globalIndex = startIndex + index;
    return selectedSet.has(globalIndex);
  }
  ...
  const isAllSelectedOnPage = currentPageData.length > 0 && 
    currentPageData.every((_, index) => isSelected(index));
        ...
        <div className="flex gap-4 mb-4">
          <div className="relative flex-grow">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" 
            />
          </div>
        </div>
        ...
        {loading && (
          <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
            <div className="bg-white/95 p-6 rounded-lg shadow-lg flex flex-col items-center pointer-events-auto">
              ...
              <div className="text-sm text-gray-700">Loading activity logs...</div>
            </div>
          </div>
        )}
